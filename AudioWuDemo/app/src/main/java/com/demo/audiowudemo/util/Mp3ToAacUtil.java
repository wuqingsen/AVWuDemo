...
public class Mp3ToAacUtil {
    ...
    //传入mp3文件 ，输出aac文件
    public Mp3ToAacUtil(String srcFile, String outFile) {
        this(srcFile, outFile, null);
    }

    public Mp3ToAacUtil(String srcFile, String outFile, OnProgressListener listener) {
        this(srcFile, outFile, -1, -1, listener);
    }

    public Mp3ToAacUtil(String srcFile, String outFile, long rangeStart, long rangeEnd, OnProgressListener listener) {
    ...
    private static class DecodeTask implements Runnable, IDataObtain {
        ...
        public void run() {
            ...
            Mp3ToAacUtil.logMsg("decodec isPrepare  " + isPrepare);
            if (isPrepare) {
                decode();
            }
            release();
        ...
        private void prepare() throws IOException {
            ...
            for (int i = 0; i < numTracks; i++) {
                ...
                if (!TextUtils.isEmpty(mine) && mine.startsWith("audio")) {
                    ...
                    codec.start();
                    Mp3ToAacUtil.logMsg("New decode codec start:" + format.toString());
                    break;
                }
            }
            ...
            while (true) {
                ...
                if (!isEOS) {
                    ...
                    if (inIndex >= 0) {
                        ...
                        Mp3ToAacUtil.logMsg("loopDecode  readSampleData end sampleSize  " + sampleSize + "    buffer.capacity()=" + buffer.capacity());
                        Mp3ToAacUtil.logMsg("loopDecode  readSampleData end timestamp" + timestamp);
...
//                Mp3ToAacUtil.logMsg(" switch (outIndex)");
                switch (outIndex) {
                    ...
                    case MediaCodec.INFO_OUTPUT_FORMAT_CHANGED:
                        ...
                        Mp3ToAacUtil.logMsg("New format " + mf.toString());
                        break;
                    ...
                    default:
                        ...
                        Mp3ToAacUtil.logMsg("解码时间：" + (now - last) + " info.size  " + info.size);
                        last = now;
    ...
    private static void logMsg(String msg) {
        Log.d(Mp3ToAacUtil.class.getSimpleName(), msg);
    }
        ...
        private void encode() {
            ...
            while (true) {
                ...
                if (outIndex >= 0) {
                    ...
                    long now = System.currentTimeMillis();
                    Mp3ToAacUtil.logMsg("编码码时间：" + (now - last) + " info.size  " + info.size);
                    last = now;
                    while (outIndex >= 0) {
