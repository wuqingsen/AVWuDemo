...
//        TXDataSourceUtils.shareInstance().setBufferSize(min_buffer_size);
...
//        int readBytes;
        byte[] pcmBuffer = new byte[SAMPLES_PER_FRAME];
...
//                } catch (InterruptedException e1) {
//                }
                int pcmBufferSize = audioRecord.read(pcmBuffer, 0,
                        SAMPLES_PER_FRAME);
                //录音解码
//                int pcmBufferSize = audioRecord.read(new short[pcmBuffer.length/2], 0,
//                        SAMPLES_PER_FRAME/2);
                if (pcmBufferSize == AudioRecord.ERROR_INVALID_OPERATION) {
                    break;
                } else if (pcmBufferSize == AudioRecord.ERROR_BAD_VALUE) {
                    break;
                } else if (pcmBufferSize != SAMPLES_PER_FRAME) {
                    break;
                }

                //音频流识别
//                if (!SpeechRealUtils.isReading && !TXUtils.isSuccess) {
//                    TencentAsrStream.shareInstance().writeByte(bytesToShort(pcmBuffer));
//                }
...
//
//                TXDataSourceUtils.shareInstance().appendData(readBytes);
...
//        Log.e("=====音频录制", "Audio 录制线程 退出...");
    ...
    private synchronized void encode(byte[] data) {
        ...
        if (data != null) {
            int bufferIndexId = mMediaCodec.dequeueInputBuffer(10000);
            if (bufferIndexId >= 0) {
                ByteBuffer inputBuffer = mMediaCodec.getInputBuffer(bufferIndexId);
                inputBuffer.clear();
                inputBuffer.put(data);
                mMediaCodec.queueInputBuffer(bufferIndexId, 0, data.length, System.nanoTime() / 1000, 0);
            }
        }
